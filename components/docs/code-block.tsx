...
function CodeBlock({
  ...
  return (
    <div
      className={cn(
        "group/codeblock relative rounded-xl border border-zinc-200 bg-zinc-950 overflow-hidden dark:border-zinc-800/80",
        className
      )}
    >
      {/* ── Header bar ────────────────────────────────── */}
      <div className="flex items-center justify-between border-b border-zinc-800/80 bg-zinc-900/50 px-4 py-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Filename tab */}
          <div className="flex items-center gap-1.5 rounded-md bg-zinc-800/60 px-2.5 py-1">
            {isTerminal ? (
      ...
      >
        <div
          className={cn(
            "overflow-x-auto p-4 text-[13px] leading-[1.7] font-mono",
      ...
      {collapsible && needsCollapse && (
        <button
          ...
          className="flex w-full items-center justify-center gap-1.5 border-t border-zinc-800/60 bg-zinc-900/30 py-2 text-xs font-medium text-zinc-500 transition-colors hover:bg-zinc-900/60 hover:text-zinc-300"
        >
          {isExpanded ? (
            <>
...
function TerminalBlock({ command, className }: TerminalBlockProps) {
  ...
  return (
    <div
      className={cn(
        "group/terminal flex items-center justify-between gap-3 rounded-lg border border-zinc-200 bg-zinc-950 px-4 py-3 dark:border-zinc-800",
        className
      )}
    >
      <div className="flex items-center gap-2.5 overflow-hidden min-w-0">
        <TerminalIcon className="size-3.5 shrink-0 text-zinc-600" />
        <code className="truncate text-[13px] font-mono text-zinc-300">
          {command}
        </code>
      </div>
      <button
