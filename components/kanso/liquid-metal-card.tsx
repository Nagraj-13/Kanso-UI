...
export interface LiquidMetalCardHeadingProps
  extends Omit<React.ComponentPropsWithoutRef<"div">, "title"> {
  title?: React.ReactNode
  subtitle?: React.ReactNode
...
export interface LiquidMetalCardDescriptionProps
  extends React.ComponentPropsWithoutRef<"div"> {
  description?: React.ReactNode
  descriptionClassName?: string
}

export interface LiquidMetalCardActionsProps
  extends React.ComponentPropsWithoutRef<"div"> {
  showCta?: boolean
  ctaProps?: Partial<LiquidMetalCardCTAProps>
  renderCta?: (defaultCta: React.ReactNode) => React.ReactNode
...
export interface LiquidMetalCardBadgesProps
  extends React.ComponentPropsWithoutRef<"div"> {
  showBadges?: boolean
  techStack?: LiquidMetalCardTechItem[]
  renderBadge?: (
...
export interface LiquidMetalCardVisualProps
  extends React.ComponentPropsWithoutRef<"div"> {
  desktopShaderProps?: Partial<LiquidMetalProps>
  desktopClassName?: string
}

export interface LiquidMetalCardMobileVisualProps
  extends React.ComponentPropsWithoutRef<"div"> {
  mobileShaderProps?: Partial<LiquidMetalProps>
}
...
export function LiquidMetalCardContent({
  ...
  return (
    <div
      className={cn(
        "flex flex-col gap-2 text-balance",
        className
      )}
        ...
        <div className="relative">
          ...
          >
            ...
            {resolvedSubtitle ? (
              <>
                {" "}
                <span className="text-zinc-400 dark:text-zinc-500 font-normal">/ {resolvedSubtitle}</span>
              </>
            ) : null}
          </h2>
        </div>
...
export function LiquidMetalCardCTA({
  ...
  return (
    ...
    >
      <Button
        render={(buttonProps) => (
          <a href={href} onClick={onClick} rel={rel} target={target} {...buttonProps}>
            {label}
          </a>
        )}
        className={cn(
      ...
      {resolvedTechStack.map((tech, index) => {
        ...
        const defaultBadge = (
          <Badge
            ...
            variant="outline"
          >
            {Icon ? <Icon className="size-3 opacity-80 mr-1.5 shrink-0" /> : null}
