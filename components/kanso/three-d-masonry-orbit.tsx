...
export function ThreeDMasonryOrbit({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        'relative w-full overflow-hidden select-none bg-zinc-50 dark:bg-[#09090b]',
        className
      )}
      style={{ height }}
      {...props}
    >
      ...
      {!hideUI && (
        <div className="absolute bottom-10 left-0 right-0 z-10 pointer-events-none flex flex-col items-center gap-2 text-center text-zinc-500/80 dark:text-white/60">
          ...
          <p className="m-0 text-xs tracking-wider">{descText}</p>
        </div>
      )}
    </div>
  );
...
const DEFAULT_PROJECTS = [
  {
    title: 'Contemplative Solitude',
    category: 'Portrait',
    year: '2025',
    client: 'Atlas Studio',
    desc: 'A quiet study in stillness — soft window light and the unhurried grace of being alone.',
    image:
      'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=600&h=800',
  },
  {
    title: 'Dynamic Basketball',
    category: 'Sports',
    year: '2025',
    client: 'Court Co.',
    desc: 'Peak athleticism frozen mid-air — explosive vertical leap and raw competitive energy.',
    image:
      'https://images.unsplash.com/photo-1546519638-68e109498ffc?auto=format&fit=crop&q=80&w=600&h=800',
  },
  {
    ...
    year: '2026',
    client: 'Form Atelier',
    desc: 'Restrained material palette and sculptural negative space, every line intentional.',
    image:
      'https://images.unsplash.com/photo-1600210492486-724fe5c67fb0?auto=format&fit=crop&q=80&w=600&h=800',
  },
  {
    title: 'Rustic Charm Café',
    category: 'Editorial',
    year: '2023',
    client: 'Lyon Press',
    desc: 'Weathered wood, warm ceramics, and the slow ritual of a European morning coffee.',
    image:
      'https://images.unsplash.com/photo-1554118811-1e0d58224f24?auto=format&fit=crop&q=80&w=600&h=800',
  },
  {
    title: 'Serene Beachcomber',
    category: 'Travel',
    year: '2025',
    client: 'Tideline',
    desc: 'Bare feet on cool sand at first light — a solitary walk along the tide.',
    image:
      'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&q=80&w=600&h=800',
  },
  {
    title: 'Grassland Portrait',
    category: 'Portrait',
    year: '2025',
    client: 'Field Notes',
    desc: 'Wind-swept grasses frame a quiet figure under an open, endless sky.',
    image:
  ...
  },
  {
    ...
    year: '2024',
    client: 'Atlas Studio',
    desc: 'A silhouette wrapped in amber light — intimacy rendered in shadow and warmth.',
    image:
  ...
  },
  {
    ...
    year: '2025',
    client: 'Court Co.',
    desc: 'The split-second of contact — tension, focus, and explosive precision.',
    image:
      'https://images.unsplash.com/photo-1595435934249-5df7ed86e1c0?auto=format&fit=crop&q=80&w=600&h=800',
  },
];
...
function buildTiles(
  s: CurvedRingSettings,
  projects: typeof DEFAULT_PROJECTS
): { tiles: Tile[]; anglePer: number } {
  ...
  for (let r = 0; r < s.rows; r++) {
    for (let c = 0; c < s.perRow; c++) {
      ...
      let projectIndex = Math.floor(
        prng(seed * 8.31 + r * 17.3 + c * 3.77) * projects.length
      );
      if (n > 0 && projectIndex === tiles[n - 1]?.projectIndex) {
        projectIndex =
          (projectIndex +
            1 +
            Math.floor(prng(seed * 2.17) * (projects.length - 1))) %
          projects.length;
      }
...
export function CurvedRingArchive({
  ...
  const activeTheme =
    (mounted ? (resolvedTheme as 'light' | 'dark') : 'dark') || 'dark';
      ...
      if (peTick % 4 === 0) {
        ...
        cardsRef.current.forEach((card, i) => {
          ...
          if (dimming) {
            ...
            card.style.opacity = (
              fade * mapRange(front, -1, 1, 0.12, 1)
            ).toString();
  ...
  return (
    <div
      ...
      className={cn(
        ...
        embedded
          ? 'h-[650px] rounded-xl border border-zinc-200 dark:border-zinc-800'
              ...
              >
                <div className="relative p-7 md:p-8">
                  <div className="flex items-center gap-3 mb-5">
                    <span className="px-2.5 py-0.5 text-[9px] font-semibold tracking-wider uppercase rounded-md border border-zinc-200 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400 bg-zinc-50 dark:bg-zinc-900">
                      {active.project.category}
                    </span>
                    <span className="text-xs font-mono tracking-widest text-zinc-400 dark:text-zinc-500">
                      {active.project.year}
                    </span>
                  </div>

                  <h2 className="text-2xl font-semibold mb-4 tracking-tight leading-snug text-zinc-950 dark:text-zinc-50">
                    {active.project.title}
                  </h2>

                  <p className="text-[14px] mb-8 leading-relaxed max-w-sm text-zinc-500 dark:text-zinc-400">
                    {active.project.desc}
                  </p>

                  <div className="grid grid-cols-2 gap-6 mb-8 pb-7 border-b border-zinc-150 dark:border-zinc-900">
                    <div>
                      <p className="text-[9px] uppercase tracking-wider text-zinc-400 dark:text-zinc-500 mb-1">
                        Client
                      </p>
                    ...
                    <div>
                      ...
                      <p className="text-xs font-mono font-medium text-zinc-800 dark:text-zinc-200">
                        {(
                          projects.findIndex(
                            (p) => p.title === active.project.title
                          ) + 1
                        )
                          .toString()
                          .padStart(3, '0')}
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <button className="inline-flex items-center gap-2 px-5 py-2.5 rounded-md text-xs font-semibold border transition-all hover:bg-zinc-900 hover:text-zinc-50 dark:hover:bg-zinc-50 dark:hover:text-zinc-955 border-zinc-200 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300 bg-transparent cursor-pointer">
                      Explore Case Study
                      <svg
                        width="12"
                        height="12"
